...
class Verify {
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            for ($i = 0; $i<$this->length; $i++) {
                // 验证码字体随机颜色
                $this->_color = imagecolorallocate($this->_image, mt_rand(1,200), mt_rand(1,200), mt_rand(1,200));
    ...
    private function _writeNoise() {
        ...
        $fontface = dirname(__FILE__) . '/Verify/zhttfs/SimHei.ttf';
        for($i = 0; $i < 10; $i++){
            //杂点颜色
            $noiseColor = imagecolorallocate($this->_image, mt_rand(0,200), mt_rand(0,120), mt_rand(0,120));
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagettftext($this->_image, mt_rand(10, 12), mt_rand(-100, 100), (60 * $i + 30), mt_rand(20, 50), $noiseColor, $fontface, $codeSet[mt_rand(0, 29)]);
            }
        }
    }
